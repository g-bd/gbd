...
public class RunDrtTlvmRandomEranBatch {
...
//TODO POP FILE 3 CHANGE OUTPUT FOLDER FILL RUN CHANGE ALPHA BETA change iters to 15
	//change run id to 15?
...
//		config.qsim().setNumberOfThreads(15);
		...
		config.controler().setOutputDirectory(
				OUTPUT_FOLDER+"batch_alpha_"+alpha+"_beta_"+((int)beta)+"_max_"+((int)maxWait)+"\\"+ runId + "_tlvm_random_distributed_"+savSeats+"_seater_" + savVehicles + "_sav_taxis_no_tta_iters_"+iters);
